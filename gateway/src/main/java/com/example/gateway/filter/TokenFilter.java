...
public class TokenFilter implements GlobalFilter, Ordered {
    ...
    // A AT pede que as consultas de fornecedores sejam acessadas direto pela
    // porta 8085, sem falar em autenticacao. Entao os GET do fornecedores-service
    // passam sem token; o resto do sistema continua protegido.
    private static final String CONSULTA_FORNECEDORES = "/fornecedores-service/fornecedores";
    ...
    public Mono<Void> filter(ServerWebExchange exchange, GatewayFilterChain chain) {
        ...
        // Consulta de fornecedores (so leitura) tambem passa direto.
        if (HttpMethod.GET.equals(exchange.getRequest().getMethod())
                && caminho.startsWith(CONSULTA_FORNECEDORES)) {
            return chain.filter(exchange);
        }
